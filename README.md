...
**macOS:** open the DMG and drag Vo-Coder to Applications. Pick the **arm64** DMG for Apple Silicon (M-series) or the **x64** DMG for Intel. On first launch, **right-click the app → Open → Open** — a plain double-click is blocked because the build isn't notarized (there is no Apple Developer account behind it). If macOS still says the app is *"damaged"*, run this once in Terminal and open it again:
...
Vo-Coder is built by one person and ships as a compiled app — the source is not published. Releases up to
**1.4.1** were open source under the old MIT-with-attribution license, and that grant stands for those
versions; everything from here on is closed. The installers stay free.

## License

Free to download and use, for personal and commercial work, on as many machines as you like. The installers
may not be redistributed or resold, and the source is not published. See [LICENSE](LICENSE).
