...
**1.2.30** — **A hard look at our own locks — and a memory that keeps the conversation.** A full security review pass: real path containment everywhere an agent names a file (the prefix-match escape and the save-as `..` escape are closed), SSRF-proof web fetching (v4-mapped IPv6 spellings included, unrecognized forms fail closed), one shared permission gate across chat, Telegram and missions (read-only tools no longer get denied on unattended missions; spending still always asks), and memory-off agents refused at execution instead of just unadvertised. The **context window learned what to keep**: the conversation. One tool-heavy turn used to evict everything that was ever *said*; now dialogue survives verbatim as far back as the budget reaches while old tool bulk is elided, full replays are bounded by the model's actual window (no more silent front-truncation on small local models), and Telegram chats stopped replaying months of history every message. **Anthropic requests are prompt-cached** (tool schemas and system prompt at ~10% on every tool round-trip) and extended-thinking replays carry their signatures, so thinking+tools no longer 400s. A **queued message can be taken back** before the agent sees it — and honestly reports queued → seen. Local endpoints that leak raw chat-template tokens as text get scrubbed and named. And `skill_read`, `project_create`, `payment_spend`, `team_clean` — advertised but unroutable after a prefix-list drift — reach their executor again, matched by name so the list can never drift.
